<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __("Update your account's profile information, email address, role, and profile photo.") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            <x-input-label for="photos" :value="__('Profile Photo')" />
            <input id="photos" name="photos" type="file" class="mt-1 block w-full" accept="image/*" />
            @if ($user->photos)
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    {{ __('Current photo') }}: <a href="{{ asset('storage/' . $user->photos) }}" target="_blank" class="underline">{{ $user->photos }}</a>
                </p>
            @endif
